...
from server1 import __init__
from server1.__init__ import Base
from server1.crud import get_pair,create_pair
from server1.schemas import PairCreate, Pair
...
from server1.database import SessionLocal, engine
...
@app.post("/pairs/", response_model=Pair)
def create_user(pair: PairCreate, db: Session = Depends(get_db)):
    db_pair = get_pair(db, pair_id=pair.pair_id)
    if db_pair:
